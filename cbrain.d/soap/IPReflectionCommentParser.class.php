	...
	function parseTagLine($tagLine) {
		...
		switch (strtolower($tag)) {
			...
			case 'param':
				...
				$o->doc      = trim(implode(" ", array_slice($tagArr, 3)));
				$o->optional = strpos($o->comment, "[OPTIONAL]") !== FALSE;
				$o->doc      = str_replace("[OPTIONAL]", "", $o->doc);
				$this->obj->params[] = $o;
				break;
			case 'return':
				$this->obj->return = array("type"=>trim($tagArr[1]),"comment"=>implode(" ", $tagArr),"doc"=>trim(implode(" ", array_slice($tagArr, 2))));
				break;
			case 'link':break;
			case 'see':break;
			case 'since':
			...
			case 'var':
				...
				$comment = implode(" ", array_slice($tagArr, 2));
				...
				break;
			case 'version':
				if ( preg_match('/\$Revision.+?(\d+).+\$/',$tagLine,$m) ) {
					$this->obj->version = $m[1];
				} else {
