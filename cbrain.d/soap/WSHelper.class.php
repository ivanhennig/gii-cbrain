...
class WSHelper {
	...
	public function __construct($uri, $class=null){
		$this->uri = $uri;
		$this->setWSDLCacheFolder($_SERVER['DOCUMENT_ROOT'].dirname($_SERVER['PHP_SELF'])."/");
	...
	public function handle(){
		//file_put_contents("_Debug.log", $_SERVER['QUERY_STRING'],FILE_APPEND);
	...
	public function showWSDL(){
		if (defined("WSIgnorarVerificacaoClasses") && WSIgnorarVerificacaoClasses) {
			//Nada
		} else {
		...
		//Encontrar uma boa solução para armazenamento em cache
	 ...
	 * @return void
	 */
	public function createDocumentation($template = "soap/docclass.xsl")	{
		if(!is_file($template))
		...
		if (defined("WSOcultarClasses") && WSOcultarClasses) {
			//Nada...
		} else {
