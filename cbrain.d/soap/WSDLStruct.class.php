		...
		foreach((array)$this->services as $serviceName => $service){
			...
			foreach((array)$service->methods as $operation){
				...
				if (is_array($operation->return)) {
					$lreturn = $operation->return['type'];
				} else {
					$lreturn = $operation->return;
				}
				if(!$lreturn || trim($lreturn) == "") {
					throw new WSDLException('No return type for '.$operationName);
				}
				if(strtolower(trim($lreturn))!='void'){
					...
					$this->addMessage($messageName,Array($operation->name."Return" => $lreturn));
				}
			}
